# 🚀 Улучшенный линтер - Современные практики

## 📋 Обзор

Этот документ описывает улучшенную систему линтинга, основанную на практиках крупных компаний (Google, Uber, Netflix, Microsoft).

## 🎯 Конфигурации

### 1. **Базовая конфигурация** (`.golangci-compatible.yml`)

- **Назначение**: Совместимость и стабильность
- **Использование**: CI/CD, проверки перед релизом
- **Линтеры**: 7 базовых линтеров
- **Время выполнения**: ~3-5 минут

### 2. **Улучшенная конфигурация** (`.golangci-enhanced.yml`)

- **Назначение**: Современные практики разработки
- **Использование**: Ежедневная разработка
- **Линтеры**: 25+ линтеров
- **Время выполнения**: ~5-10 минут

### 3. **Строгая конфигурация** (`.golangci-strict.yml`)

- **Назначение**: Production код
- **Использование**: Критически важные компоненты
- **Линтеры**: 50+ линтеров
- **Время выполнения**: ~10-15 минут

### 4. **Быстрая конфигурация** (`.golangci-fast.yml`)

- **Назначение**: Быстрая разработка
- **Использование**: Локальная разработка
- **Линтеры**: 20 быстрых линтеров
- **Время выполнения**: ~1-3 минуты

## 🏢 Практики крупных компаний

### **Google**

- **gosec**: Проверки безопасности
- **gocritic**: Критические замечания
- **gocyclo**: Цикломатическая сложность
- **funlen**: Длина функций

### **Uber**

- **prealloc**: Предварительное выделение памяти
- **perfsprint**: Оптимизация форматирования
- **fatcontext**: Проверка контекстов
- **makezero**: Инициализация слайсов

### **Netflix**

- **testifylint**: Проверки testify
- **paralleltest**: Параллельные тесты
- **testpackage**: Изоляция тестов
- **thelper**: Тестовые хелперы

### **Microsoft**

- **gochecknoinits**: Отсутствие init функций
- **gochecknoglobals**: Отсутствие глобальных переменных
- **gomnd**: Магические числа
- **depguard**: Контроль импортов

## 🔧 Команды

### **Базовые команды**

```bash
# Базовая конфигурация
make lint

# Улучшенная конфигурация
make lint-enhanced

# Строгая конфигурация
make lint-strict

# Быстрая конфигурация
make lint-fast
```

### **Команды для отдельных сервисов**

```bash
# Bot сервис
make lint-bot

# Matcher сервис
make lint-matcher

# Profile сервис
make lint-profile
```

### **Дополнительные команды**

```bash
# Форматирование
make fmt

# Go vet
make vet

# Тесты
make test

# Очистка
make clean
```

## 📊 Категории линтеров

### **🔒 Безопасность**

- `gosec` - Проверки безопасности
- `errcheck` - Проверка ошибок
- `bodyclose` - Закрытие HTTP соединений
- `noctx` - Контекст в HTTP запросах
- `sqlclosecheck` - Закрытие SQL соединений

### **⚡ Производительность**

- `prealloc` - Предварительное выделение памяти
- `perfsprint` - Оптимизация форматирования
- `fatcontext` - Проверка контекстов
- `makezero` - Инициализация слайсов

### **🎨 Стиль кода**

- `gocritic` - Критические замечания
- `revive` - Современный golint
- `goconst` - Константы вместо магических строк
- `gocyclo` - Цикломатическая сложность
- `funlen` - Длина функций
- `gocognit` - Когнитивная сложность
- `nestif` - Вложенные if

### **🧪 Тестирование**

- `testifylint` - Проверки testify
- `paralleltest` - Параллельные тесты
- `testpackage` - Изоляция тестов
- `thelper` - Тестовые хелперы

### **📈 Качество кода**

- `gochecknoinits` - Отсутствие init функций
- `gochecknoglobals` - Отсутствие глобальных переменных
- `gomnd` - Магические числа
- `gomoddirectives` - Директивы go.mod
- `gomodguard` - Защита зависимостей
- `depguard` - Контроль импортов

## 🎯 Рекомендации по использованию

### **Для разработчиков**

1. **Локальная разработка**: `make lint-fast`
2. **Перед коммитом**: `make lint-enhanced`
3. **Перед релизом**: `make lint-strict`

### **Для CI/CD**

1. **Pull Request**: `make lint-enhanced`
2. **Main branch**: `make lint-strict`
3. **Release**: `make lint-strict`

### **Для команд**

1. **Новые проекты**: Начать с `enhanced`
2. **Legacy код**: Начать с `compatible`
3. **Критический код**: Использовать `strict`

## 📈 Метрики качества

### **Цикломатическая сложность**

- **Рекомендуется**: < 15
- **Строго**: < 10
- **Критично**: < 5

### **Длина функций**

- **Рекомендуется**: < 100 строк
- **Строго**: < 50 строк
- **Критично**: < 25 строк

### **Когнитивная сложность**

- **Рекомендуется**: < 20
- **Строго**: < 15
- **Критично**: < 10

## 🔄 Интеграция с IDE

### **VS Code**

```json
{
  "go.lintTool": "golangci-lint",
  "go.lintFlags": ["--config=services/deploy/linter/.golangci-enhanced.yml"]
}
```

### **GoLand**

1. Settings → Go → Linters
2. Выбрать golangci-lint
3. Указать конфигурацию

## 🚀 Автоматизация

### **Pre-commit hooks**

```bash
# Установка pre-commit
pip install pre-commit

# Создание .pre-commit-config.yaml
repos:
  - repo: local
    hooks:
      - id: golangci-lint
        name: golangci-lint
        entry: make lint-fast
        language: system
        files: \.go$
```

### **GitHub Actions**

```yaml
name: Lint
on: [push, pull_request]
jobs:
  lint:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3
      - uses: actions/setup-go@v3
        with:
          go-version: '1.25'
      - name: Run linter
        run: make lint-enhanced
```

## 📚 Дополнительные ресурсы

- [golangci-lint документация](https://golangci-lint.run/)
- [Go Code Review Comments](https://github.com/golang/go/wiki/CodeReviewComments)
- [Effective Go](https://golang.org/doc/effective_go.html)
- [Go Best Practices](https://github.com/golang/go/wiki/CodeReviewComments)
