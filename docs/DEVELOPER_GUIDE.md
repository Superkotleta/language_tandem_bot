...
1. **Зарегистрируйте команду в `handlers.go`**:
...
1. **Добавьте локализацию в `locales/`**:
...
1. **Добавьте в интерфейс `internal/database/interface.go`**:
...
1. **Создайте миграцию в `services/deploy/migrations/`**:
...
1. **Сделайте изменения и коммиты**:
